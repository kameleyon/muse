...
async function generateCompletion(incompleteContent, systemPrompt, chapterContext, selectedProfile, model) {
...
const generateChapter = async (req, res) => {
    try {
        ...
        const chapterResult = await supabase_1.supabaseAdmin // Use supabaseAdmin
            .from('chapters')
            .select('*')
            .eq('id', chapterId);
        ...
        if (!chapterResult.data || chapterResult.data.length === 0) {
            ...
            const allChaptersResult = await supabase_1.supabaseAdmin // Use supabaseAdmin
                .from('chapters')
        ...
        const bookResult = await supabase_1.supabaseAdmin // Use supabaseAdmin
            .from('books')
            .select('*')
        ...
        const { data: allChapters } = await supabase_1.supabaseAdmin // Use supabaseAdmin
            .from('chapters')
            .select('number, title')
...
`;
        ...
        // Use configurable model for chapter generation
        const model = config_1.default.openRouter.defaultContentModel || 'google/gemini-2.5-flash';
...
- Just write the actual chapter content continuously`;
        ...
        if (validation.needsRetry && !validation.isComplete) {
            ...
            try {
                const completion = await generateCompletion(finalContent, systemPrompt, chapter, selectedProfile, model);
        ...
        const { data: updatedChapter, error: updateError } = await supabase_1.supabaseAdmin // Use supabaseAdmin
            .from('chapters')
        ...
        await supabase_1.supabaseAdmin // Use supabaseAdmin
            .from('books')
